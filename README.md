...
Your gaming GPU is already an AI machine. ezai self-hosts LLMs on the hardware you
...
[![CI](https://github.com/MikaelSabuhi/ezaiselfhost/actions/workflows/ci.yml/badge.svg)](https://github.com/MikaelSabuhi/ezaiselfhost/actions/workflows/ci.yml)
...
| Platform | GPU used | How ezai sets it up |
| --- | --- | --- |
...
| Linux (no GPU) | CPU | Same; ezai warns you it will be slow |
| Windows + NVIDIA | CUDA | `winget install Ollama.Ollama`, OpenWebUI via `uv tool` |
| Windows (no GPU) | CPU | Same; ezai warns you it will be slow |

Ollama already installed? ezai reuses it instead of installing a second copy.

## Everyday commands

| Command | What it does |
| --- | --- |
| `ezai` | The setup wizard: detect → pick a model → install → open the browser |
| `ezai up` | Start the local AI stack and open the browser |
| `ezai down` | Stop OpenWebUI (Ollama keeps running as a shared service) |
| `ezai status` | Show what's running |
| `ezai logs` | Print the tail of ezai's log files (`--lines`, default 50) |
| `ezai doctor` | Diagnose the local setup; every ✗ comes with a one-line fix |
| `ezai update` | Upgrade Ollama and OpenWebUI to their latest versions |
| `ezai model add [ref]` | Download a model and make it the default (no ref → browse) |
| `ezai model list` | List downloaded models |
| `ezai model rm <name>` | Remove a downloaded model |

Global flags: `--version`, `--dry-run` (show what the wizard would do without doing
it), and `--help` on every command.

All state lives in one place: `~/.ezai` (config, pid files, logs). Point `EZAI_HOME`
somewhere else if you prefer.

## Pick any model

One field, three shapes. The wizard and `ezai model add` both take all of them:

```sh
ezai model add qwen3:8b                          # any tag from the Ollama library
ezai model add hf.co/unsloth/gemma-3-4b-it-GGUF  # any GGUF build on Hugging Face
ezai model add meta-llama/Llama-3.3-70B-Instruct # full-weight HF repo — needs vLLM
```

The third shape needs vLLM, which arrives with Server mode; today ezai says so and
points you at the GGUF build of the same model.

Run `ezai model add` with no argument and you get the curated list filtered to your
...
- **8 GB RAM** recommended. Less works: ezai just steers you to smaller models.
- **A GPU is optional.** CPU-only machines run everything; they run it slowly, and
  ezai tells you so up front.
- **macOS:** [Homebrew](https://brew.sh) for the Ollama install (without it, ezai
  points you at the Ollama.app download). **Windows:** winget, which ships with
  Windows 10/11.

## Why not just use Ollama directly?

You absolutely can. ezai drives [Ollama](https://ollama.com) and
...
| Something's broken | Two sets of logs, two projects' issue trackers | `ezai doctor`, one fix hint per failure |
| Staying current | Update each piece separately, per platform | `ezai update` |

If you'd rather run the raw tools, their docs are excellent and ezai gets out of your
way. And if you're curious what it's doing on your behalf, it's under a thousand lines
of plain Python in [`src/ezai/`](src/ezai): two dependencies, no magic.
