...
3. **Every external effect is an injected callable** (`run`, `which`, `popen`, `urlopen`, `sleep`, `call`, `kill`, `bind`). The test suite (187 tests) runs in under a second with no network, no Docker, and no real processes — and an autouse fixture points `LEPIKA_HOME` at a temp dir so tests can never touch a real `~/.lepika`.
...
- vLLM is Server-mode scope — planned, not present.
